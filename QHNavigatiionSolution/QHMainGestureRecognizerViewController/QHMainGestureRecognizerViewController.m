...
@property (nonatomic,strong,readwrite) NSMutableArray *arViewControllers;
...
//    [self.view setBackgroundColor:[UIColor grayColor]];
    if (!self.arViewControllers)
    {
        self.arViewControllers = [NSMutableArray new];
    }
...
- (void)addViewController2Main:(UIViewController *)viewController
{
    ...
    [self.arViewControllers addObject:viewController];
    viewController.view.tag = [self.arViewControllers count];
    ...
    if ([self.arViewControllers count] > 1)
    {
        UIViewController *backViewController = [self.arViewControllers objectAtIndex:[self.arViewControllers count] - 2];
...
- (void)popViewController2Parent
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        _isMoving = NO;
        [self.arViewControllers removeLastObject];
...
- (void)popPrepare
{
    ...
    UIView *backView = ((UIViewController *)[self.arViewControllers objectAtIndex:[self.arViewControllers count] - 2]).view;
    backView.alpha = 1;
    switch (_moveType)
    {
        case MoveTypeScale:
...
- (void)moveViewWithX:(float)x
{
    ...
    UIView *view = ((UIViewController *)[self.arViewControllers lastObject]).view;
...
{
    if (self.arViewControllers.count <= 1 || !self.canDragBack) return;
